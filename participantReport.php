...
<body>
    ...
    <main class="hours-report">
        ...
        <?php else: ?>
            ...
            <h2>
                Accommodations for 
                <?php echo htmlspecialchars($volunteer->get_first_name() . ' ' . $volunteer->get_last_name()); ?>
            </h2>

            <section>
                <h3>Sensory Sensitivities</h3>
                <?php if (!empty($sensorySensitivities)): ?>
                    <ul>
                        <?php 

                            foreach ($sensorySensitivities as $sensitivity): 
                        ?>
                            <li><?php echo htmlspecialchars($sensitivity); ?></li>
                        <?php endforeach; ?>
                    </ul>
                <?php else: ?>
                    <p>No sensory sensitivities reported.</p>
                <?php endif; ?>
            </section>

            <section>
                <h3>Disability Accommodation Needs</h3>
                <?php if (!empty($disabilityNeeds)): ?>
                    <ul>
                        <?php foreach ($disabilityNeeds as $need): ?>
                            <li><?php echo htmlspecialchars($need); ?></li>
                        <?php endforeach; ?>
                    </ul>
                <?php else: ?>
                    <p>No disability accommodation needs reported.</p>
                <?php endif; ?>
            </section>
